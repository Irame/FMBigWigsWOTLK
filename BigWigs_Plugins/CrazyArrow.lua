...
local plugin = BigWigs:NewPlugin("CrazyArrow")
...
do
	...
	function plugin:GetPluginConfig()
		...
		do
			local dist = AceGUI:Create("CheckBox")
			dist:SetValue(self.db.profile.showTitle)
			dist:SetLabel(L["Distance"])
			dist:SetCallback("OnEnter", onControlEnter)
			dist:SetCallback("OnLeave", onControlLeave)
			dist:SetCallback("OnValueChanged", checkboxCallback)
			dist:SetUserData("tooltip", L["Shows or hides the distance."])
			dist:SetUserData("key", "showTitle")
			dist:SetFullWidth(true)
			...
			showHide:AddChildren(dist, background, ability)
		end
