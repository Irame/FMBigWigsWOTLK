...
local function rearrangeBars(anchor)
	...
	if anchor == normalAnchor then
		up = db.growup
		--print("rearrangeBars()","up:",up,"(normalAnchor)")
	else 
		up = db.emphasizeGrowup 
		--print("rearrangeBars()","up:",up,"(emphasizeAnchor)")
	end
	for i, bar in next, tmp do
		bar:ClearAllPoints()
		if up then  --up or (db.emphasizeGrowup and bar:Get("bigwigs:emphasized"))
			if TukuiCF then
				bar:SetPoint("BOTTOMLEFT", lastUpBar or anchor, "TOPLEFT", 0, bar.spacingV)
				bar:SetPoint("BOTTOMRIGHT", lastUpBar or anchor, "TOPRIGHT", 0, bar.spacingV)
			else
			...
			if TukuiCF then
				bar:SetPoint("TOPLEFT", lastDownBar or anchor, "BOTTOMLEFT", 0, bar.spacingV)
				bar:SetPoint("TOPRIGHT", lastDownBar or anchor, "BOTTOMRIGHT", 0, bar.spacingV)
			else
...
do
	...
	function plugin:GetPluginConfig()
		...
		local spacingH = AceGUI:Create("Slider")
		spacingH:SetValue(db.spacingH)
		spacingH:SetSliderValues(0, 10, 1)
		spacingH:SetLabel(L["Horizontally spacing"])
		spacingH:SetUserData("key", "spacingH")
		spacingH:SetCallback("OnValueChanged", standardCallback)
		spacingH:SetFullWidth(true)
		
		local spacingV = AceGUI:Create("Slider")
		spacingV:SetValue(db.spacingV)
		spacingV:SetSliderValues(0, 10, 1)
		spacingV:SetLabel(L["Vertical spacing"])
		spacingV:SetUserData("key", "spacingV")
		spacingV:SetCallback("OnValueChanged", standardCallback)
		spacingV:SetFullWidth(true)
		
		if TukuiCF then
			return tex, font, align, icon, duration, normal, emphasize, spacingH, spacingV
		else
...
function plugin:BigWigs_StartBar(message, module, key, text, time, icon)
	...
	if TukuiCF then
		bar.candyBarBar:SetStatusBarColor(colors:GetColor("barColor", module, key))
	else
		bar:SetColor(colors:GetColor("barColor", module, key))
	end
	bar.candyBarLabel:SetTextColor(colors:GetColor("barText", module, key))
	if TukuiCF then
		bar.statusbarWindow.SetBorderColor(bar.statusbarWindow,colors:GetColor("borderColor", module, key))
		bar.statusbarWindow.SetBorderBackgroundColor(bar.statusbarWindow,colors:GetColor("borderBackground", module, key))
		bar.candyBarIconFrame.SetBorderColor(bar.candyBarIconFrame,colors:GetColor("borderColor", module, key))
		bar.candyBarIconFrame.SetBorderBackgroundColor(bar.candyBarIconFrame,colors:GetColor("borderBackground", module, key))
	end
	bar.candyBarLabel:SetJustifyH(db.align)
	...
	bar.spacingH = db.spacingH
	bar.spacingV = db.spacingV
